...
fun inverseMonotone(func: (Real) -> Real, low: Real, high: Real): (Real) -> Real {
    return InverseMonotoneUnaryRealFunction(func, low, high)
}
...
internal class InverseMonotoneUnaryRealFunction(
    ...
    init {
        ...
        maxMSD = max(abs(low), abs(high)).msd()
        maxArgPrecision = (high - low).msd() - 4
        derivMSD = ((fHigh - fLow) / (high - low)).msd()
    }

    internal inner class InverseIncreasingReal(x: Real) : Real() {
        val arg: Real = if (fNegated) x.unaryMinus() else x
    ...
    override fun invoke(argument: Real): Real {
        return InverseIncreasingReal(argument)
    }
}
